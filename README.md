...
Claude Codeのサブエージェント同士は直接コンテキストをやり取りできません。そのため、このテンプレートでは以下の方法でコンテキスト共有を実現しています：

### コンテキスト共有の仕組み
- **ファイルベースの共有**: `docs/context.md` を共通のコンテキストストアとして利用
- **各エージェントの動作**:
  1. 実行前に `docs/context.md` と `docs/plan.md` を読み込み（最新状態を取得）
  2. 自身の処理を実行
  3. 結果を `docs/context.md` に書き込み（次のエージェントへ引き継ぎ）
- **メインエージェントの役割**: サブエージェントの呼び出し順序を制御し、間接的にコンテキストの流れを管理

この設計により、サブエージェント間で状態や進捗を確実に共有でき、複雑なワークフローの実現が可能になっています。
...
- **検証範囲**: React を用いた小規模アプリ（ユーザー管理なし・DBなし・ローカル実行のみ・Dockerなし、MCPなし）で検証済みです。中規模以上への適用性は未検証です。
